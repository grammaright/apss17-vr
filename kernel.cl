__kernel void recover_video(__global unsigned char* R,
    ...
    __private int2 group_id = (int2) (get_group_id(0) + (queue_number * 30 * N), get_group_id(1));
    if (group_id.x <= group_id.y) return;

    __private int2 global_id = (int2) (get_global_id(0) + (queue_number * 960 * N), get_global_id(1));
    __private int i = global_id.x / 1920;
    __private int j = global_id.y / 1080;

    if (queue_number == 0 && ((i + j) >= N)) return;
    if (queue_number == 1 && ((i + j) < N)) return;
